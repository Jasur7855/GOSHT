...
const MainPage = ({}) => {
  ...
  const { data: achievementsData, isLoading: isAchievementsLoading } = useGetAchievementsQuery();
  const { data: informationData, isLoading: isInformationLoading } = useGetHomeInformationQuery();
  const { data: aboutBlockData, isLoading: isAboutBlockLoading } = useGetAboutBlockQuery();
  const { data: fullMenuData, isLoading: isFullMenuLoading } = useGetMenuQuery();
  ...
  if (isLoading || isMenuLoading || isAchievementsLoading || isInformationLoading || isAboutBlockLoading || isFullMenuLoading) {
    ...
    <SMainPage>
      ...
      <section className="menuInfo container">
        <div className="bgImg container">
          ...
          {achievementsData?.map((achievement, index) => (
            <div key={achievement.type} className={achievement.type === "second" ? "center" : ""}>
              <h6>
      ...
      <section className="mainMenu container">
        {visibleMenuItems.map((item) => (
          ...
          >
            <MenuCard
              cardBadge={item.tag_icon_url[0] ? `https://new.gosht.com${item.tag_icon_url[0]}` : "/icons/badge.svg"}
      ...
      {informationData?.map((info) => (
        <section key={info.id} className="allFood container">
          <div className="allFoodImg">
            ...
            <img src={`https://new.gosht.com${info.image}`} alt={info.title} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
          </div>
